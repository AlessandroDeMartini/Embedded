...
enum active {on = 2, off = 1};    // what does it mean 2 and 1
...
{ 
  ...
  INT16S velocity = 0; 
  INT16S wind_factor;   
  enum active brake_pedal = off;   //perchè???
...
Timer1 = OSTmrCreate (0,                              	// number that is the initial delay
                      3,                              	// number that is the amount of time it will take before the timer expires
                      OS_TMR_OPT_PERIODIC,             	// should chose between OS_TMR_OPT_PERIODIC that stand for periodic or OS_TMR_OPT_ONE_SHOT only once
                      callbackVehicle,                 	// callback function ?? shold be defined prior OSTmrStop()
                      NULL,                           	// as before (argument of callback function)
                      NULL,			                      	// pointer that allow to give a name to the timer (also NULL)
                      &err);			                     	// error (&err) 

OSTmrStart(Timer1, &err) 					// to actually start the timer

Timer2 = OSTmrCreate (0,                              	// number that is the initial delay
                      3,                              	// number that is the amount of time it will take before the timer expires
                      OS_TMR_OPT_PERIODIC,             	// should chose between OS_TMR_OPT_PERIODIC that stand for periodic or OS_TMR_OPT_ONE_SHOT only once
                      callbackOverload,                	// callback function ?? shold be defined prior OSTmrStop()
                      NULL,                           	// as before (argument of callback function)
                      NULL,			                      	// pointer that allow to give a name to the timer (also NULL)
                      &err);			                     	// error (&err) 

OSTmrStart(Timer2, &err) 					// to actually start the timer
